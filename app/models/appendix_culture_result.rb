class AppendixCultureResult < ApplicationRecord
  ...
  validates :first_pathogen_specify, presence: {message: 'Please specify.'}, if: Proc.new{|f| f.first_pathogen == 'Other enterobacteriaceae.' || f.first_pathogen == 'Other non-fermenter.' || f.first_pathogen == 'Other gram-positive cocci.' || f.first_pathogen == 'Other bacteria' || f.first_pathogen == 'Other fungi.'}
  validates :first_pathogen_enter_cfu_ml, inclusion: {in: 1..5, message: 'Please select.'}

  validates :second_pathogen, presence: {message: 'Please select.'}, if: Proc.new{|f| f.second_pathogen.present?}
  validates :second_pathogen_specify, presence: {message: 'Please specify.'}, if: Proc.new{|f| f.second_pathogen == 'Other enterobacteriaceae.' || f.second_pathogen == 'Other non-fermenter.' || f.second_pathogen == 'Other gram-positive cocci.' || f.second_pathogen == 'Other bacteria' || f.second_pathogen == 'Other fungi.' }
  validates :second_pathogen_enter_cfu_ml, inclusion: {in: 1..5, message: 'Please select.'}, if: Proc.new{|f| f.second_pathogen.present?}
    ...
    [:first, :second].each do |place|
      SUSCEPTIBILITY_FIELDS.each do |group|
        ...
        if group.has_key?(:sub_fields)
          ...
          unless self.send("#{place}_#{group[:field]}")
            # If not, set all pathogens to nil
            group[:sub_fields].each do |sub_field|
              self.send("#{place}_#{sub_field[:field]}=", nil)
            end
          end

          # Set all resistance values to nil, where the pathogen is not selected
          group[:sub_fields].each do |sub_field|
            unless self.send("#{place}_#{sub_field[:field]}")
              self.send("#{place}_#{sub_field[:field]}_s=", nil)
            end
          end
        else
          # Single pathogen
          unless self.send("#{place}_#{group[:field]}")
            self.send("#{place}_#{group[:field]}_s=", nil)
          end
        end
      end
    end
