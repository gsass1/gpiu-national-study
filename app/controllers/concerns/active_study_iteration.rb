...
module ActiveStudyIteration
  ...
  def current_study_iteration
    @current_study_iteration ||=
      (current_user.nil? ? nil : current_user.country.current_study_iteration)
  end

  def next_or_current_study_iteration
    @next_or_current_study_iteration ||=
      (current_user.nil? ? nil : current_user.country.next_or_current_study_iteration)
  end

  def check_current_study_iteration!
    return if study_active? || current_user.admin?
