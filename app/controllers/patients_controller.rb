class PatientsController < ApplicationController
  ...
  def create
    @patient = Patient.new(patient_params.merge(creator_id: current_user.id, study_iteration_id: current_study_iteration.id))
  ...
  def load_study_iteration
    if study_active?
      @active_range = current_study_iteration.active_range
    end
  end
